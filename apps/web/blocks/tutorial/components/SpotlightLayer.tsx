...
export function SpotlightLayer({
  ...
  React.useEffect(() => {
    ...
    // Check if navigation is needed (for Next.js routing)
    if (onNavigation && step.path) {
      const currentPath = typeof window !== 'undefined' ? window.location.pathname : '';
      if (step.path !== currentPath) {
        onNavigation(step.path);
        return; // Don't open spotlight until navigation completes
      }
    }
  ...
  }, [running, step, openForRef, stepAutoFocus, onNavigation]);
